    ...
    if (minmonthpos != NULL)
    {
        ...
        { /*Stunde bzw. Jahr überspringen*/
            ...
            {              /*Jahr*/
                ...
                while (p[0] == ' ')
                    p++;
                if (wcslen(p) > 5 && DecodeNumber(&p, &num))
                {        /*Stunde überspringen*/
                    p--; /*Achtung: nicht Namen als Zahl erkennen!*/
                    ch = p[0];
                    p++;
                    if (num <= 24 && num >= 0 && (ch == ':' || (ch == '.' && isadigit(p[0]))) && DecodeNumber(&p, &num))
                    {        /*Zeit*/
                        p--; /*Achtung: nicht Namen als Zahl erkennen!*/
                        ch = p[0];
                        p++;
    ...
    *sizefile = -1;
                ...
                do
                {
                    ...
                    if (monthpos != lpstr)
                    {
                        if (wcschr(L" \t-,/", monthpos[3]) == NULL)
                        {
                            ...
                            lr-xr-xr-t	 1 root			 sys						 20 16. März, 15:20 endif ->
                            /opt/ansic/bin/endif*/
                            if (wcschr(L" \t-,/", monthpos[4]) != NULL)
                            {
                                found = true;
    ...
    if (monthpos)
    {
        ...
        if (lp)
        {
            ...
            {
                lp1 = lp;
                while (isadigit(lp1[0]))
                    lp1++;

                while (wcschr(L" \t-,./", lp1[0]) != NULL)
                { /*lp1 zeigt nun auf Zeit bzw. Jahr*/
                    lp1[0] = 0;
                    lp1++;
                }
                t.wDay = _wtoi(lp);
                     ...
                     DWORD *UnixAttr, BOOL longdatetype)
{
    ...
    *UnixAttr = 0;
    ...
    _wcsupr_s(testbuf, _countof(testbuf));
