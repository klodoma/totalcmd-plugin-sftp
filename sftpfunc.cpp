	...
	if (szFP != NULL)
	{
		while (szFP != NULL)
		{
			...
			if (szFP == NULL && strlen(szOld) > 0)
				pv->push_back(MAKELONG((WORD)(szOld - szIn), (WORD)(&szIn[strlen(szIn)] - szOld))); // add pair with pos and length
		}
	}
}
             ...
             void **abstract)
{
	...
	for (int i=0;i<num_prompts;i++) {
		...
		BOOL autoSendPassword=(ConnectSettings && ConnectSettings->protectedpassword.length>0 && !ConnectSettings->InteractivePasswordSent);
		if (autoSendPassword) {
			_strlwr(retbuf);
...
void libssh2_trace_handler_callback(LIBSSH2_SESSION* session,
                                           void* param,
                                           const char *error,
                                           size_t len)
{
...
int SftpConnect(pConnectSettings ConnectSettings)
{
	...
	if (!ConnectSettings->session) {
		...
		libssh2_trace_sethandler(ConnectSettings->session,
                                         NULL,
                                         libssh2_trace_handler_callback);
		...
		for (int method=0;method<=9;method++) {
			...
			GetPrivateProfileString(ConnectSettings->DisplayName,buf,"",prefs,sizeof(prefs)-1,ConnectSettings->IniFileName);
			if (prefs[0]) {
				while ((err=libssh2_session_method_pref(ConnectSettings->session,method,prefs))== LIBSSH2_ERROR_EAGAIN) {
					if (ProgressLoop(buf,30,40,&loop,&lasttime))
						break;
					IsSocketReadable(ConnectSettings->sock);  // sleep to avoid 100% CPU!
				}
		...
		if(libssh2_userauth_authenticated(ConnectSettings->session)) {
			...
			if (hf==INVALID_HANDLE_VALUE) {
				...
				if (ReadFile(hf,&filebuf,35,&dataread,NULL)) {
					if (_strnicmp(filebuf,"ssh-",4)!=0 && _strnicmp(filebuf,"ecdsa-",6)!=0 && _strnicmp(filebuf,"-----BEGIN OPENSSH PRIVATE KEY-----",35)!=0) {
	...
	case WM_INITDIALOG: {
		...
		if (strcmp(gDisplayName,s_quickconnect)!=0) {
			...
			if (gConnectResults->protectedpassword.length==-1 && CryptProc) {
				password[0]=0;
				if (/*!gConnectResults->useagent &&*/ CryptProc(PluginNumber,CryptoNumber,FS_CRYPT_LOAD_PASSWORD_NO_UI,gDisplayName,password,countof(password)-1)==FS_FILE_OK) {
	...
	case WM_COMMAND: {
		switch(LOWORD(wParam)) {
					...
					if (!gConnectResults->dialogforconnection)
					{
						...
						if (!IsWindowVisible(GetDlgItem(hWnd,IDC_EDITPASS))) {
							if (gConnectResults->protectedpassword.length==0) {
								...
								DecodeProtectedPassword(password,&gConnectResults->protectedpassword);
								BOOL ok=CryptProc(PluginNumber,CryptoNumber,FS_CRYPT_SAVE_PASSWORD,gDisplayName,password,0)==FS_FILE_OK;
			...
			{	
				...
				int err;
				DecodeProtectedPassword(password,&gConnectResults->protectedpassword);
				err=CryptProc(PluginNumber,CryptoNumber,FS_CRYPT_LOAD_PASSWORD,gDisplayName,password,countof(password)-1);
				if (err==FS_FILE_OK) {
					EncodeProtectedPassword(&gConnectResults->protectedpassword,password);
					SetDlgItemText(hWnd,IDC_PASSWORD,password);
				} else if (err=FS_FILE_READERROR) {         // no password stored!
			...
			{
				TCHAR szCaption[100]; LoadString(hinst, IDS_HELP_CAPTION, szCaption, countof(szCaption));
				TCHAR szBuffer[1024]; LoadString(hinst, IDS_HELP_PASSWORD, szBuffer, countof(szBuffer));
				MessageBox(hWnd, szBuffer, szCaption, MB_OK | MB_ICONINFORMATION);
				break;
			}
			case IDC_UTF8HELP: 
...
BOOL ShowConnectDialog(pConnectSettings ConnectSettings,char* DisplayName,char* inifilename)
{
	...
	if (ConnectSettings->dialogforconnection && ConnectSettings->server[0]) {
		if ((ConnectSettings->user[0]==0 ||
			ConnectSettings->protectedpassword.length>0) &&        // password saved
...
void* SftpConnectToServer(char* DisplayName,char* inifilename,pProtectedPassword overridepass)
{
	...
	if (ShowConnectDialog(&ConnectSettings,DisplayName,inifilename)) {
		...
		if (CryptProc && (gConnectResults->protectedpassword.length==-1)) {
			...
			char password[MAX_PASSWORD];
			password[0]=0;
			if (CryptProc(PluginNumber,CryptoNumber,FS_CRYPT_LOAD_PASSWORD,gDisplayName,password,countof(password)-1)!=FS_FILE_OK) {
...
BOOL SftpFindNextFileW(void* serverid,void* davdataptr,WIN32_FIND_DATAW *FindData,char skippedchar)
{
	...
	if (ConnectSettings->scponly) {
		...
		if (skippedchar) {                // character received while requesting directory
			completeline[0]=skippedchar;
			completelineptr++;
			maxlen--;
		}
		while (ReadChannelLine(channel,completelineptr,maxlen,scpd->msgbuf,sizeof(scpd->msgbuf)-1,
		...
		if (strchr("-DLFBCP|", buf[0]))
		{
			...
			if (v.size() >= 4 && IsNumeric(&completeline[LOWORD(v[1])], HIWORD(v[1])))
			{
				...
				if (info) {
					...
					if (file.flags & LIBSSH2_SFTP_ATTR_PERMISSIONS)
					{
						info->szPermissions[0] = (file.permissions & LIBSSH2_SFTP_S_IFLNK) == LIBSSH2_SFTP_S_IFLNK ? 'L' : '-';
						info->szPermissions[1] = (char)48;
						info->szPermissions[2] = (char)48;
						info->szPermissions[3] = (char)48;

						// RWXRWXRWX
						if (file.permissions & LIBSSH2_SFTP_S_IRUSR) info->szPermissions[1] += 4;
						if (file.permissions & LIBSSH2_SFTP_S_IWUSR) info->szPermissions[1] += 2;
						if (file.permissions & LIBSSH2_SFTP_S_IXUSR) info->szPermissions[1] += 1;

						if (file.permissions & LIBSSH2_SFTP_S_IRGRP) info->szPermissions[2] += 4;
						if (file.permissions & LIBSSH2_SFTP_S_IWGRP) info->szPermissions[2] += 2;
						if (file.permissions & LIBSSH2_SFTP_S_IXGRP) info->szPermissions[2] += 1;

						if (file.permissions & LIBSSH2_SFTP_S_IROTH) info->szPermissions[3] += 4;
						if (file.permissions & LIBSSH2_SFTP_S_IWOTH) info->szPermissions[3] += 2;
						if (file.permissions & LIBSSH2_SFTP_S_IXOTH) info->szPermissions[3] += 1;
				...
				if (p) {
					...
					if (p3 && p3-p>=4) {
						...
						if (p3>p && p3[2]==' ' && p3[3]=='p' ||        //    cp: invalid option - p
							p3[0]==' ' && p3[2]=='p') {                 // cp: invalid option -- 'p'
...
// the doc says we should use at least 32K but internally libssh2 limits the size to 30K which leads to more packets than needed
// see discussion: https://github.com/libssh2/libssh2/discussions/1064
...
{	
		...
		if (!SSH_ScpNeedQuote && remotefilescp==0 && strchr(filename,' ')!=0 &&
			...
			if (EscapeWithDoubleQuotes(filename2,filename,sizeof(filename2)-1)>0) {
				...
				if (remotefilescp)
					...
					libssh2_session_flag(ConnectSettings->session, LIBSSH2_FLAG_QUOTE_PATHS, 1);  // let the library handle it again
			}
		}
		...
		if (!remotefilescp || fileinfoscp.st_size==-1) {
			...
			SftpLogLastError("SCP download error: ",libssh2_session_last_errno(ConnectSettings->session));
				...
				if (rcerr>0) {
					...
					LogProc(PluginNumber,MSGTYPE_IMPORTANTERROR,"SCP download error: No size returned for file.");  // Dropbear - crashes if we don't check tghis!
	...
	if (Resume) {
		...
		localfile=CreateFileT(LocalName,GENERIC_WRITE,FILE_SHARE_READ | FILE_SHARE_WRITE,NULL,
			alwaysoverwrite ? OPEN_ALWAYS : CREATE_NEW,      // new: only truncate once the download has started, to not lose the old file on error
	...
	do {
		...
		if (current - start >= 1000 && lenSum >= maxblocksize) {
			...
			if (maxblocksize != maxblocksizeOld && maxblocksizeOld < maxblocksize) {
				...
				//char errbuf[1024];
				//sprintf(errbuf, "Download block size changed: %i bytes, old: %i bytes", maxblocksize, maxblocksizeOld);
				//LogProc(PluginNumber, MSGTYPE_IMPORTANTERROR, errbuf);
			}
			else
				maxblocksize = maxblocksizeOld;
			...
			while (ReadFile(localfile,data,maxblocksize,&len,NULL) && len>0) {
				...
				if (current - start >= 1000 && writtenSum >= maxblocksize) {
					...
					if (maxblocksize != maxblocksizeOld && maxblocksizeOld < maxblocksize) {
						...
						//char errbuf[1024];
						//sprintf(errbuf, "Upload block size changed: %i bytes, old: %i bytes", maxblocksize, maxblocksizeOld);
						//LogProc(PluginNumber, MSGTYPE_IMPORTANTERROR, errbuf);
					}
					else
						maxblocksize = maxblocksizeOld;
		...
		if (!g_command_ls) {
			...
			if (p) {
				...
				if (strlen(p)>20 && p[4]=='-' && p[7]=='-' && p[10]==' ' && p[13]==':' && p[16]==':' && p[19]=='.') {
					...
					p[19]=' ';
					tdt.wMilliseconds=0;
					sscanf(p,"%hd %hd %hd %hd %hd %hd",&tdt.wYear,&tdt.wMonth,&tdt.wDay,&tdt.wHour,&tdt.wMinute,&tdt.wSecond);
					p2=strchr(p+19,'-');
					if (!p2) {
						p2=strchr(p+19,'+');
						if (p2)
