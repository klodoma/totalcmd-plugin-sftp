...
typedef struct
{
    ...
    // linux useradd allows up to 32 chars
    char szUser[33];
    char szGroup[33];
    char szPermissions[5];

    WCHAR wszLinkDestination[MAX_PATH];

} tFileInfoExtra, *pFileInfoExtra;

#define MAX_PASSWORD 3 * MAX_PATH

typedef struct
{
    char password[MAX_PASSWORD];
    int length;
} tProtectedPassword, *pProtectedPassword;
...
typedef struct
{
    ...
    char user[MAX_PATH];
    tProtectedPassword protectedpassword;
    ...
    BOOL wantKeepTimestamp; // keep timestamp when copying remotely
    ...
    HWND hWndKeepAlive;
    int debuglog;       // sum of LIBSSH2_TRACE_* flags
    int scpserver64bit; // 0=no, 1=yes, -1, auto-detect -> Support file upload/download > 2GB only if SCP on server side
                        // is 64bit! There might be 32bit SCP implementations with large file support but we cannot
                        // detect it.
